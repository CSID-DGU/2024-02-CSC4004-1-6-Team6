...
def remove_josa_and_duplicates(text):
    # 한글 조사의 정규식 (예: 가, 이, 를, 은 등)
    josa_pattern = re.compile(r'\b[가-힣]+(이|가|를|은|는|에|의|와|과|도|으로|로|에서)\b')

    # 1. 조사 제거
    text_without_josa = josa_pattern.sub(lambda m: m.group().rstrip('이가를은는에의와과도로에서'), text)
...
# 알고리즘 실행
processed_text = remove_josa_and_duplicates(diary_text)
print("처리된 텍스트:")
