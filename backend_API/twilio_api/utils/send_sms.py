...
# 환경 변수 로드, todo : 프로젝트 시작점에서 한번만 호출되어야함, 나중에 위치 옮겨야할듯
load_dotenv()
...
def send_message(phone_number, text):
    ...
    message = client.messages.create(
        ...
        to=phone_number  # 수신자 번호, 체험판에서는 인증된 번호에만 보낼 수 있음
    )
    return message.sid


# 사용자에게 경고 문자 발송 - 트리거는 주간 감정분석중 주간 작성한 일기가 3장 이상일 때
def emotion_alarm_for_user(user, threshold=80):
    recent_week = WeeklyEmotion.objects.filter(user=user).order_by("-week_start").first()

    if recent_week.sadness >= threshold:
        # 메시지 내용
        message_body = (
            "아래의 기관에 연락하는 것을 권장드립니다.\n"
            "기관 연락처 : xxx-xxxx-xxxx\n"
            f"주간 슬픔 상태 : {recent_week.sadness}점"
        )
        send_message(user.phone_number, message_body)
        print(f"{user.username}님에게 주간 데이터를 기반으로 메시지를 전송했습니다.")
    else:
        print(f"현재 {user.username}님의 감정 상태는 위험 수준이 아닙니다.")
